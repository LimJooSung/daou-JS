...
<script>
	...
	/* 유효성 검증을 위한 함수 (버튼 클릭 시 실행됨) */
	function checkForm() {
		// document의 loginForm의 각 값을 저장하는 변수 선언
		...
		// 사번 유효성 체크 (사번은 5자라고 가정(특수문자도 입력 가능하도록( ex. 11-7101 등 )))
		if (empNo == '') {
			alert("사번을 입력하세요.");
...
<div class="login_wrap login_b">
    ...
    <div class="login_box">
        <div class="form">
            <form name="loginForm" class="login_form" action="${rc.contextPath}/loginProc.do" method="post" >
                ...
                <div class="btn_box"><button class="login" onclick="return checkForm()">로그인</button></div>
            </form>
        </div>
    </div>
</div>
